classdef nsoltIntermediateLieExpMap2dLayerTestCase < matlab.unittest.TestCase
    %NSOLTINTERMEDIATELIEEXPMAP2DLAYERTESTCASE 
    %   
    ...
    methods (TestClassTeardown)
        function finalCheck(~)
            import saivdr.dcnn.*
            layer = nsoltIntermediateLieExpMap2dLayer(...
    ...
    methods (Test)
        ...
        function testConstructor(testCase, nchs)
            ...
            expctdDescription = "Synthesis NSOLT intermediate ExpMap " ...
                + "(ps,pa) = (" ...
                + nchs(1) + "," + nchs(2) + ")";
        ...
        function testPredictGrayscaleWithRandomWeights(testCase, ...
            ...
            %X = randn(nrows,ncols,nChsTotal,nSamples,datatype);
            X = randn(nChsTotal,nrows,ncols,nSamples,datatype);
            weights = randn((nChsTotal-2)*nChsTotal/8,1);
            
            % Expected values
            % nChsTotal x nRows x nCols x nSamples
            ps = nchs(1);
            pa = nchs(2);
            UnT = transpose(genU.step(weights,mus));
            Y = X; %permute(X,[3 1 2 4]);
            Ya = reshape(Y(ps+1:ps+pa,:,:,:),pa,nrows*ncols*nSamples);
            Za = UnT*Ya;
            Y(ps+1:ps+pa,:,:,:) = reshape(Za,pa,nrows,ncols,nSamples);
        ...
        function testPredictGrayscaleAnalysisMode(testCase, ...
            ...
            ps = nchs(1);
            pa = nchs(2);
            Un = genU.step(weights,mus);
            Y = X; % permute(X,[3 1 2 4]);
            Ya = reshape(Y(ps+1:ps+pa,:,:,:),pa,nrows*ncols*nSamples);
            Za = Un*Ya;
            Y(ps+1:ps+pa,:,:,:) = reshape(Za,pa,nrows,ncols,nSamples);
            expctdZ = Y; %ipermute(Y,[3 1 2 4]);
            expctdDescription = "Analysis NSOLT intermediate ExpMap " ...
                + "(ps,pa) = (" ...
                + nchs(1) + "," + nchs(2) + ")";
        ...
        function testBackwardGrayscale(testCase, ...
            ...
            nSamples = 8;
            nChsTotal = sum(nchs);
            nWeights = (nChsTotal-2)*nChsTotal/8;
            weights = zeros(nWeights,1,datatype);
            ...
            % dLdWi = <dLdZ,(dVdWi)X>
            expctddLdW = zeros(nWeights,1,datatype);
            for iAngle = 1:nWeights
                dUn_T = transpose(genU.step(weights,mus,iAngle));
        ...
        function testBackwardGrayscaleAnalysisMode(testCase, ...
            ...
            % dLdX = dZdX x dLdZ
            UnT = transpose(genU.step(weights,mus,0));
            adLd_ = dLdZ; %permute(dLdZ,[3 1 2 4]);
            cdLd_low = reshape(adLd_(ps+1:ps+pa,:,:,:),pa,nrows*ncols*nSamples);
            ...
            expctddLdW = zeros(nWeights,1,datatype);
            for iAngle = 1:nWeights
                dUn = genU.step(weights,mus,iAngle);
                a_ = X; %permute(X,[3 1 2 4]);
                c_low = reshape(a_(ps+1:ps+pa,:,:,:),pa,nrows*ncols*nSamples);
                c_low = dUn*c_low;
                a_ = zeros(size(a_),datatype);
