classdef nsoltInitialLieExpMap2dLayerTestCase < matlab.unittest.TestCase
    %NSOLTINITIALLIEEXPMAP2DLAYERTESTCASE
    %
    ...
    methods (TestClassTeardown)
        function finalCheck(~)
            import saivdr.dcnn.*
            layer = nsoltInitialLieExpMap2dLayer(...
    ...
    methods (Test)
        ...
        function testConstructor(testCase, nchs, stride)
            ...
            % Expected values
            expctdName = 'V0';
            expctdDescription = "NSOLT initial ExpMap " ...
                + "(ps,pa) = (" ...
                + nchs(1) + "," + nchs(2) + "), "  ...
                + "(mv,mh) = (" ...
                + stride(1) + "," + stride(2) + ")";
        ...
        function testPredictGrayscaleWithRandomWeights(testCase, ...
            ...
            ps = nchs(1);
            pa = nchs(2);
            W0 = genW.step(weights(1:length(weights)/2),1);
            U0 = genU.step(weights(length(weights)/2+1:end),1);
            %expctdZ = zeros(nrows,ncols,nChsTotal,nSamples,datatype);
            expctdZ = zeros(nChsTotal,nrows,ncols,nSamples,datatype);
            Y  = zeros(nChsTotal,nrows,ncols,datatype);
            for iSample=1:nSamples
        ...
        function testPredictGrayscaleWithRandomWeightsNoDcLeackage(testCase, ...
            ...
            musW(1,1) = 1;
            musU = mus*ones(pa,1);
            W0 = genW.step(weightsNoDc(1:length(weights)/2),musW);
            U0 = genU.step(weightsNoDc(length(weights)/2+1:end),musU);
            %expctdZ = zeros(nrows,ncols,nChsTotal,nSamples,datatype);
            expctdZ = zeros(nChsTotal,nrows,ncols,nSamples,datatype);
            Y  = zeros(nChsTotal,nrows,ncols,datatype);
            for iSample=1:nSamples
        ...
        function testBackwardGrayscale(testCase, ...
            ...
            % dLdX = dZdX x dLdZ
            W0T = transpose(genW.step(weightsW,mus_,0));
            U0T = transpose(genU.step(weightsU,mus_,0));
            ...
            for iAngle = 1:nWeightsH
                dW0 = genW.step(weightsW,mus_,iAngle);
                dU0 = genU.step(weightsU,mus_,iAngle);
        ...
        function testBackwardGrayscaleWithRandomWeights(testCase, ...
                nchs, stride, nrows, ncols, datatype)
        ...
        function testBackwardGrayscaleWithRandomWeightsNoDcLeackage(testCase, ...
                nchs, stride, nrows, ncols, mus, datatype)
            ...
            musW(1,1) = 1;
            musU = mus*ones(pa,1);            
            W0T = transpose(genW.step(weightsW_NoDc,musW,0));
            U0T = transpose(genU.step(weightsU,musU,0));
            Y = dLdZ; %permute(dLdZ,[3 1 2 4]);
            Ys = reshape(Y(1:ps,:,:,:),ps,nrows*ncols*nSamples);
            Ya = reshape(Y(ps+1:ps+pa,:,:,:),pa,nrows*ncols*nSamples);
            Zsa = [ W0T(1:ceil(nDecs/2),:)*Ys; U0T(1:floor(nDecs/2),:)*Ya ];
            ...
            % (dVdWi)X
            for iAngle = 1:nWeightsH
                dW0 = genW.step(weightsW_NoDc,musW,iAngle);
                dU0 = genU.step(weightsU,musU,iAngle);
                a_ = X; %permute(X,[3 1 2 4]);
                c_upp = reshape(a_(1:ceil(nDecs/2),:,:,:),ceil(nDecs/2),nrows*ncols*nSamples);
                c_low = reshape(a_(ceil(nDecs/2)+1:nDecs,:,:,:),floor(nDecs/2),nrows*ncols*nSamples);
                d_upp = dW0(:,1:ceil(nDecs/2))*c_upp;
