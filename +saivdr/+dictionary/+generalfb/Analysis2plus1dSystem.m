        ...
        function setupImpl(obj,srcImg)
            ...
            if isFrequency_
                ...
                for iLevel = 1:nLevels
                    dec_ = obj.DecimationFactor(1:2).^(iLevel-1);
                    % DCT for Z direction
                    for iCol = 1:nCols_
                        for iRow = 1:nRows_
                            freqRes_(iRow,iCol,:,iSubband) = dct(...
                                freqRes_(iRow,iCol,:,1));
                        end
                    end
                    % X-Y filtering
                    for iCh = nChs_:-1:2
                        ...
                        for iLay = 1:nLays_
                            freqRes_(:,:,iLay,iSubband) = bsxfun(@times,...
                                freqRes_(:,:,iLay,1),...
                                fftn(hext,[nRows_,nCols_]));
                        end
                        iSubband = iSubband - 1;
                    end
                    h    = obj.upsample2_(...
    ...
    methods (Access = private)
        ...
        function [coefs,scales] = analyzeSpatial_(obj,srcImg)
            ...
            for iLevel = 1:nLevels
                ...
                for iCol = 1:width
                    for iRow = 1:height
                        dctImg(iRow,iCol,:) = dct(subImg(iRow,iCol,:));
                    end
                end
                % X-Y filtering
                for iCh = nChs_:-1:2
