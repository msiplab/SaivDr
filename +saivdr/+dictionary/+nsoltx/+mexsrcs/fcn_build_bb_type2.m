function [fcnhandler,flag] = fcn_build_bb_type2()
...
if license('checkout','matlab_coder') % Coder is available
    ...
    if fbsfile == 2
        ...
        aPs        = coder.typeof(double(0),1,0); %#ok
        aPa        = coder.typeof(double(0),1,0); %#ok
        nshift     = coder.typeof(int32(0),1,0); %#ok
        % build mex
        cfg = coder.config('mex');
        cfg.DynamicMemoryAllocation = 'AllVariableSizeArrays';%'Threshold';
        cfg.GenerateReport = true;
        args = '{ arrayCoefs, paramMtxW, paramMtxU, aPs, aPa, nshift }';
