classdef PolyPhaseCoefOperation2dSystem < matlab.System %#codegen
    %POLYPHASECOEFOPERATION2DSYSTEM 2-D polyphase matrix
    %
    % SVN identifier:
    % $Id: PolyPhaseCoefOperation2dSystem.m 683 2015-05-29 08:22:13Z sho $
    %
    ...
    methods
        ...
        function obj = PolyPhaseCoefOperation2dSystem(varargin)
    ...
    methods (Access = protected)
        ...
        function validateInputsImpl(obj,varargin)
            if strcmp(obj.operationCategory,'Unary')
                input1 = varargin{1};
                flag1 = isnumeric(input1) && ndims(input1) <= 4;
                if ~flag1
                    error('Input must be numeric less than or equal to 4-D.')
                end
            elseif strcmp(obj.operationCategory,'Binary')
                input1 = varargin{1};
                input2 = varargin{2};
                flag1 = isnumeric(input1) && ndims(input1) <= 4;
                flag2 = isnumeric(input2) && ndims(input2) <= 4;
                if ~flag1 || ~flag2
                    error('Input must be numeric less than or equal to 4-D.')
                end
            ...
            elseif strcmp(obj.Operation,'Upsample')
                ...
                flag1 = isnumeric(input) && ndims(input) <= 4;
                if ~flag1
                    error('Input must be numeric less than or equal to 4-D.')
                end                
                ...
                if ~isnumeric(direction) || any(direction < 1) || any(direction > 2)
                    error('Direction must be either of 1 or 2.')
                end
            else
