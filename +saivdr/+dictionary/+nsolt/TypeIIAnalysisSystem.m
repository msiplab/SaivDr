classdef TypeIIAnalysisSystem < saivdr.dictionary.nsolt.AbstNsoltAnalysisSystem %#~codegen
    %TYPEIIANALYSISSYSTEM Analysis system of Type-II NSOLT
    %
    % SVN identifier:
    % $Id: TypeIIAnalysisSystem.m 683 2015-05-29 08:22:13Z sho $
    %
    ...
    methods
        ...
        % Constructor
        function obj = TypeIIAnalysisSystem(varargin)
            obj = ...
    ...
    methods (Access=protected)
        ...
        function setupImpl(obj,coefs,scales)
            ...
            if ~hflag
                obj.supportExtensionHorizontalObj ...
                    = saivdr.dictionary.nsolt.mexsrcs.SupportExtensionHorizontalTypeII();
            ...
            [obj.supportExtensionVerticalFcn,   vflag] = fcn_autobuild(...
                'fcn_SupportExtensionVerticalTypeII',...
                obj.NumberOfSymmetricChannels,...
                obj.NumberOfAntisymmetricChannels);
            if ~vflag
                obj.supportExtensionVerticalObj ...
                    = saivdr.dictionary.nsolt.mexsrcs.SupportExtensionVerticalTypeII();
        ...
        function obj = analyze_(obj,srcImg)
            ...
            width = size(srcImg,2);
            obj.nRows = int32(height/obj.decY);
            obj.nCols = int32(width/obj.decX);
            blockSize = zeros(1,2);
            ...
            elseif obj.decY == 2 && obj.decX == 2
                ...
                dctCoefs = blockproc(srcImg,blockSize,...
                    @(x) TypeIIAnalysisSystem.dct2_(x));
                dctCoefs = blockproc(dctCoefs,blockSize,...
                    @(x) TypeIIAnalysisSystem.permuteDctCoefs_(x));
            end
            ...
            obj.arrayCoefs(hLen_(1)+1:hLen_(1)+mf,:) = coefs(mc+1:end,:);
            ...
            for iOrd = uint32(1):obj.ordX/2  % Horizontal process
                paramMtx1 = step(obj.paramMtx,[],2*iOrd+1); % W
                paramMtx2 = step(obj.paramMtx,[],2*iOrd+2); % U
            ...
            for iOrd = uint32(1):obj.ordY/2  % Vertical process
                paramMtx1 = ...
                    step(obj.paramMtx,[],2*iOrd+obj.ordX+1); % W
                paramMtx2 = ...
                    step(obj.paramMtx,[],2*iOrd+obj.ordX+2); % U
