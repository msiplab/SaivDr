...
class NsoltCopySynthesis2AnalysisTestCase(unittest.TestCase):
    """
    NSOLTCOPYSYNTHESIS2ANALYSISTESTCASE Test cases for Nsolt{Analysis,Synthesis}2dNetwork
    ...
    def testAdjointOfSynthesis2Network(self,
        ...
        synthesizer.apply(init_angles)

        # Adjoint 
        analyzer = synthesizer.T
    ...
    def testAdjointOfSynthesis2dNetworkRandomInitialization(self,
        height,width,nchs,stride,ppord,nlevels,nvm,datatype):
        rtol,atol = 1e-3,1e-6
        if isdevicetest:
            device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")   
