function [h,f,E] = fcn_type1d222c55(ord,angs,mus,isnodc)
%
% $Id: fcn_type1d222c55.m 850 2015-10-29 21:19:55Z sho $
%
ps = 5;
pa = 5;
nChs = ps + pa;
...
if nargin < 3
    mus  = [ ones(5,1) ; -ones(5,1) ;
            repmat( [ -ones(5,1) ; ones(5,1)],[3 1])];
end
...
omg5 = OrthonormalMatrixGenerationSystem();
...
%% E0
if isnodc
    angs(1:ps-1) = zeros(ps-1,1);
    mus(1)    = 1;
end
W0  = step(omg5,angs(aidx:aidx+nAngW-1),mus(midx:midx+nMusW-1));
aidx = aidx+nAngW;
midx = midx+nMusW;
%
U0  = step(omg5,angs(aidx:aidx+nAngU-1),mus(midx:midx+nMusU-1));
aidx = aidx+nAngU;
midx = midx+nMusU;
%
...
%% Vertical extension
for ordY = 1:ord(1)
    U = step(omg5,angs(aidx:aidx+nAngU-1),mus(midx:midx+nMusU-1));
    aidx = aidx+nAngU;
    midx = midx+nMusU;
    R = blkdiag(eye(ps),U);
    E = R*Qy*E;
